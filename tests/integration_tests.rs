...
async fn test_session_workflow() {
    ...
    // Create a session
    let session = session_manager.create_session().await.expect("Failed to create session");
    ...
    session_manager.add_message(&session.id, message).await.expect("Failed to add message");
    
    // Retrieve the updated session
    let updated_session = session_manager.get_session(&session.id).await
...
async fn test_tool_calls_and_results() {
    ...
    let deserialized_call: ToolCall = serde_json::from_str(&serialized_call).expect("Failed to deserialize tool call");
    
    assert_eq!(deserialized_call.id, tool_call.id);
    assert_eq!(deserialized_call.name, tool_call.name);
    
    let serialized_result = serde_json::to_string(&tool_result).expect("Failed to serialize tool result");
    let deserialized_result: ToolResult = serde_json::from_str(&serialized_result).expect("Failed to deserialize tool result");
...
async fn test_concurrent_session_access() {
    ...
    let session_manager = std::sync::Arc::new(
        llama_agent::session::SessionManager::new(config.session_config)
    );
    ...
    for handle in handles {
        let session = handle.await.expect("Task panicked").expect("Session creation failed");
    ...
    for (i, session) in sessions.iter().enumerate() {
        let retrieved = session_manager.get_session(&session.id).await
            .expect("Failed to get session")
            .expect("Session not found");
        
        assert_eq!(retrieved.messages.len(), 3);
        for (j, message) in retrieved.messages.iter().enumerate() {
            assert!(message.content.contains(&format!("Message {} from task {}", j, i)));
        }
    }
}
