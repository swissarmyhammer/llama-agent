...
impl TestSetup {
    ...
    async fn new() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let backend = match LlamaBackend::init() {
            ...
            Err(_) => {
                ...
                return Err(
                    "Backend already initialized - integration tests require fresh process".into(),
                );
            }
        };
    ...
    fn generate_tokens(
        ...
        prompt: &str,
        max_tokens: usize,
    ) -> Result<(Vec<llama_cpp_2::token::LlamaToken>, String), Box<dyn std::error::Error>> {
    ...
    fn create_context(
        &self,
    ) -> Result<llama_cpp_2::context::LlamaContext, Box<dyn std::error::Error>> {
        let context_params = LlamaContextParams::default()
...
async fn test_stopper_performance_benchmark() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Baseline: {:.2} tokens/sec, {} tokens in {:?}",
    ...
    let (with_stoppers_metrics, _) = PerformanceMetrics::measure(|| {
        ...
        for chunk in tokens.chunks(5) {
            ...
            for (i, &token) in chunk.iter().enumerate() {
                batch
                    .add(token, i as i32, &[0], i == chunk.len() - 1)
                    .unwrap();
            }
    ...
    info!(
        "With stoppers: {:.2} tokens/sec, {} tokens in {:?}",
    ...
    if performance_impact > 0.0 {
        assert!(
            ...
            "Performance impact ({:.2}%) exceeds 5% requirement",
            performance_impact
        );
        info!(
            "✓ Performance impact {:.2}% is within acceptable limits",
            performance_impact
        );
    } else {
