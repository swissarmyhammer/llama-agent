...
mod tests {
    ...
    fn test_max_tokens_validation() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("exceeds security limit"));
    ...
    fn test_temperature_validation() {
        ...
        let result = validator.validate(&session, &request);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("between 0 and 2"));
    ...
    fn test_custom_config() {
        ...
        let result = validator.validate(&session, &request);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("1001"));
