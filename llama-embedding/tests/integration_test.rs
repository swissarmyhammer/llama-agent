use llama_embedding::{EmbeddingConfig, EmbeddingModel, BatchProcessor};
...
async fn test_embedding_model_creation() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            println!("Model creation failed (expected in test environment): {}", e);
        }
    }
}
...
fn test_embedding_config() {
    ...
    match config.model_source {
        ModelSource::HuggingFace { ref repo, ref filename } => {
...
fn test_embedding_result() {
    ...
    assert!((magnitude - 1.0).abs() < 1e-6, "Vector should be normalized to unit length");
}
